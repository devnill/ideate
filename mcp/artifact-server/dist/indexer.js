...
const fileToCacheKeys = new Map();
function registerDependency(cacheKey, filePath) {
...
// ---------------------------------------------------------------------------
const embeddingIndexes = new Map();
/** Track which artifactDir a given absolute file path belongs to, for watcher re-index */
const fileToArtifactDir = new Map();
function getOrCreateEmbeddingIndex(artifactDir) {
...
artifactWatcher.on("change", ({ artifactDir, filePath }) => {
    const idx = embeddingIndexes.get(artifactDir);
    if (!idx)
        return; // Index not yet created for this dir — nothing to update
    // Run async without blocking the event loop; errors are non-fatal
    reindexFile(idx, filePath).catch(() => { });
});
async function reindexFile(idx, filePath) {
    let content;
    try {
...
function deriveChunks(filePath, content) {
 ...
 */
async function ensureIndexed(artifactDir, sourceDir) {
    ...
    if (embeddingIndexes.has(sentinelKey)) {
        return idx;
    }
    embeddingIndexes.set(sentinelKey, true);
    ...
    const toIndex = artifactFiles.filter((f) => f.endsWith(".md") || f.endsWith(".yaml") || f.endsWith(".yml"));
    // Gather source files
...
async function globDir(dir, options = {}) {
    const results = [];
    if (!existsSync(dir))
        return results;
...
function classifyArtifact(filePath, artifactDir) {
    const rel = path.relative(artifactDir, filePath);
    if (rel.startsWith("plan/work-items"))
        return "work-item";
    if (rel.startsWith("plan/modules"))
        return "module-spec";
    ...
    if (rel.startsWith("domains/"))
        return "domain";
    if (rel.startsWith("archive/incremental"))
        return "incremental-review";
    if (rel.startsWith("archive/cycles"))
        return "cycle-review";
...
function isSourceFile(filePath) {
    ...
    if (parts.some((p) => SOURCE_EXCLUDES.includes(p)))
        return false;
 ...
 */
export async function artifactIndex(artifactDir) {
 ...
 */
export async function domainPolicies(artifactDir, domain) {
    ...
    for (const domainName of domainDirs.sort()) {
        ...
        if (content) {
            ...
            lines.push(`# Domain: ${domainName}`, "", `Note: ${policiesPath} not found.`, "", "---", "");
        }
    }
 ...
 */
export async function sourceIndex(artifactDir, sourceDir, filterPath) {
    ...
    for (const filePath of sourceFiles.sort()) {
        ...
        if (exports.length === 0)
            continue; // skip files with no detectable exports
 ...
 */
export async function getContextPackage(artifactDir, reviewScope, changedFiles) {
    ...
    const principlesPath = path.join(artifactDir, "steering", "guiding-principles.md");
...
async function detectSourceDir(artifactDir) {
...
function extractExportsN(content, language, n) {
    const pattern = EXPORT_PATTERNS[language];
    if (!pattern)
        return [];
    const exports = [];
 ...
 */
export async function getWorkItemContext(artifactDir, workItemId) {
    ...
    const moduleSpec = await loadRelatedModuleSpec(artifactDir, workItemContent, cacheKey);
    if (moduleSpec) {
    ...
    if (relevantDomains.length > 0) {
        for (const domain of relevantDomains) {
            const policiesPath = path.join(artifactDir, "domains", domain, "policies.md");
            registerDependency(cacheKey, policiesPath);
            const content = await readFileSafe(policiesPath);
            if (content) {
                sections.push("", `## Domain Policies: ${domain}`, "", content.trim());
            }
        }
    }
    else {
...
async function loadWorkItem(artifactDir, workItemId, cacheKey) {
...
function extractWorkItemFromYaml(yaml, id) {
...
async function loadRelatedModuleSpec(artifactDir, workItemContent, cacheKey) {
...
function extractMentionedDomains(content) {
    const matches = content.match(/domains?\/([\w-]+)/gi) ?? [];
    return [
        ...new Set(matches.map((m) => m.replace(/^domains?\//i, "").toLowerCase())),
    ];
}
async function loadResearch(artifactDir, cacheKey) {
 ...
 */
export async function artifactQuery(artifactDir, query) {
    ...
    for (let i = 0; i < top.length; i++) {
        ...
        result.push(`## Result ${i + 1}: ${relPath} (lines ${chunk.startLine}–${chunk.endLine})`, "", chunk.text, "", "---", "");
    }
    return result.join("\n");
}
// ---------------------------------------------------------------------------
...
export async function artifactSemanticSearch(args) {
    ...
    for (let i = 0; i < results.length; i++) {
        ...
        lines.push(`## [${finalScore.toFixed(3)}] ${citation} — ${section}`, `_semantic: ${semanticScore.toFixed(3)}, bm25: ${bm25Score.toFixed(3)}, type: ${chunk.artifactType}${chunk.domain ? `, domain: ${chunk.domain}` : ""}_`, "", chunk.content, "", "---", "");
    }
    return lines.join("\n");
}
