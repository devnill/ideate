...
vi.mock("@xenova/transformers", () => ({
    pipeline: vi.fn().mockResolvedValue(async (text, _options) => ({
        data: hashToVector(text),
    })),
}));
import { artifactIndex, domainPolicies, artifactQuery, buildSourceCodeIndex, } from "../indexer.js";
// -----------------------------------------------------------------------
// Helpers
// -----------------------------------------------------------------------
let tmpDir;
...
describe("indexer tool functions", () => {
    beforeEach(() => {
        ...
        writeFixture("steering/guiding-principles.md", "# Guiding Principles\n\nAlways write tests before code.\nKeep modules small and focused.\nPrefer composition over inheritance.");
        writeFixture("steering/constraints.md", "# Constraints\n\nMust support Node 20+.\nNo external API calls in tests.\nMaximum 500ms response time.");
        writeFixture("plan/architecture.md", "# Architecture\n\n## Components\n\nThe system has three layers:\n- API layer\n- Service layer\n- Data layer");
        writeFixture("plan/work-items.yaml", [
            "- id: 001",
            "  title: Setup project",
            "  status: done",
            "- id: 002",
    ...
    describe("artifactIndex", () => {
        it("returns JSON listing all files in the artifact directory", async () => {
            ...
            expect(filePaths).toContain(path.join("domains", "test-domain", "policies.md"));
        });
        it("includes file type classification", async () => {
            const result = await artifactIndex(tmpDir);
            const parsed = JSON.parse(result);
            const steering = parsed.files.find((f) => f.path === "steering/guiding-principles.md");
            expect(steering).toBeDefined();
            expect(steering.type).toBe("steering");
            const plan = parsed.files.find((f) => f.path === "plan/architecture.md");
    ...
    describe("buildSourceCodeIndex", () => {
        it("truncates output at maxRows and appends an omission note", async () => {
            ...
            for (let i = 0; i < 10; i++) {
                writeFixture(`src/module${i}.ts`, `export function func${i}() {}\n`);
            }
            const result = await buildSourceCodeIndex(`test-cachekey-${tmpDir}`, path.join(tmpDir, "src"), 5, 3 // cap at 3 rows
            );
        ...
        it("returns all rows when file count is under maxRows", async () => {
            ...
            const result = await buildSourceCodeIndex(`test-cachekey-small-${tmpDir}`, path.join(tmpDir, "src"), 5, 10 // cap well above file count
            );
