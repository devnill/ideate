...
function createMockIndex(
  candidates: Array<{ chunk: Chunk; similarity: number }>
): EmbeddingIndex {
  return {
...
describe("semanticSearch", () => {
  ...
  it("boosts principles with similarity > 0.3", async () => {
    ...
    const principleResult = results.find(
      (r) => r.chunk.artifactType === "principle"
    )!;
    const otherResult = results.find(
      (r) => r.chunk.artifactType === "other"
    )!;
  ...
  it("does not boost principles with similarity <= 0.3", async () => {
    ...
    expect(principleResult.finalScore).toBeCloseTo(
      otherResult.finalScore,
      5
    );
  });
