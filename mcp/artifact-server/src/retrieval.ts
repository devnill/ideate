...
function normalizeScores(scores: Map<string, number>): Map<string, number> {
  ...
  if (max === 0) return new Map(Array.from(scores.entries()).map(([k]) => [k, 0]));
...
export async function semanticSearch(
  ...
  const normSemantic = new Map<string, number>();
  ...
  // 5. Combine scores
  const results: SearchResult[] = candidates.map(({ chunk, similarity }) => {
    ...
    if (
      (chunk.artifactType === "principle" || chunk.artifactType === "constraint") &&
      similarity > 0.3
    ) {
